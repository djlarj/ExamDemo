...
// Quiz Data: Generate 10 addition problems
const quizData = [];
for (let i = 0; i < 10; i++) {
    const num1 = Math.floor(Math.random() * 13); // Random number between 0 and 12
    const num2 = Math.floor(Math.random() * 13); // Random number between 0 and 12
    ...
    while (question.options.length < 4) {
        const incorrectAnswer = Math.floor(Math.random() * 25); // Random number between 0 and 24
